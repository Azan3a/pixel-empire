...
function WorldGridInner({ tintR = 1, tintG = 1, tintB = 1 }: WorldGridProps) {
  const drawGrid = useCallback(
    (g: Graphics) => {
      ...
      g.fill({ color: t(0x4a7a4a) });

      g.setStrokeStyle({ color: t(0x3d6b3d), width: 1, alpha: 0.35 });
      ...
      for (let bx = 0; bx < MAP_SIZE; bx += ROAD_SPACING) {
        for (let by = 0; by < MAP_SIZE; by += ROAD_SPACING) {
          ...
          for (let ti = 0; ti < 4; ti++) {
            ...
            const size = 6 + seeded(bx, by, ti * 3 + 2) * 6;
            ...
            g.circle(tx, ty, size);
            g.fill({ color: t(0x2d8a2d), alpha: 0.75 });

            g.circle(tx - 1, ty - 1, size * 0.6);
            g.fill({ color: t(0x3ca03c), alpha: 0.5 });

            g.circle(tx, ty, 2);
            g.fill({ color: t(0x5c3a1a) });
          }
        }
      }
