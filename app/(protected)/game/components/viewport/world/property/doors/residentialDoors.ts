// components/game/viewport/world/property/doors/residentialDoors.ts
...
export function drawApartmentEntrance(
  ...
  // Push bars
  const barW = doorW / 2 - 4;
  g.rect(doorX + 2, doorY + doorH * 0.4, barW, 1.5);
  g.fill({ color: tintColor(0xaaaaaa, bf), alpha: 0.5 });
  g.rect(doorX + doorW / 2 + 2, doorY + doorH * 0.4, barW, 1.5);
  g.fill({ color: tintColor(0xaaaaaa, bf), alpha: 0.5 });

  if (isNight) {
    g.rect(doorX - 2, doorY - 1, doorW + 4, doorH + 2);
    g.fill({ color: 0xfff8e0, alpha: 0.08 });
  }
}
